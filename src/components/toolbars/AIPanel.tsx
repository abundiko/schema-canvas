...
export function AIPanel() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Something went wrong. Nothing was generated.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="absolute right-3 top-14 z-30 flex h-[min(560px,calc(100%-72px))] w-80 flex-col overflow-hidden rounded-xl border border-border bg-white shadow-floating dark:bg-zinc-900">
      ...
      <div className="scrollbar-thin flex-1 space-y-3 overflow-y-auto p-3">
        {tab === "generate" ? (
          <>
            ...
            {proposal && !applied && (
              <div
                className="rounded-lg border border-brand-200 bg-brand-50/60 p-2.5 dark:border-brand-500/30 dark:bg-brand-500/10"
              >
      ...
      <div className="border-t border-border px-3 py-1.5 text-[10px] text-text-faint">
        Schema suggestions are generated by AI on the server. Set your language-model API key server-side to enable generation.
      </div>
    </div>
  );
}
